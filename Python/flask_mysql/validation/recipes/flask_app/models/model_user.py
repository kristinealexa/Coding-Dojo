...
class User:
    ...
    def validate(data: dict) -> bool:
        ...
        if len(data["last_name"]) < 2:
            ...
            is_valid = False
        if len(data["email"]) < 7:
            flash("email is required, must be at least 7 characters long","err_users_email")
            is_valid = False
        elif not EMAIL_REGEX.match(data["email"]):
            flash("Invalid email address!", "err_users_email")
            is_valid = False
        if len(data["password"]) < 8:
            flash("password is required, must be at least 8 characters long","err_users_password")
            is_valid = False
        if len(data["confirm_password"]) < 8:
            flash("confirm_password is required, must be at least 8 characters long","err_users_confirm_password")
            is_valid = False
